<?php
	...
	if($_GET['cue']==2){
		...
		$a3=$_POST['a3'];
		$a4=$_POST['a4'];
		$sqlmedia = "SELECT * FROM media WHERE id=47";
	...
	if($row1['status']==5){
		...
		mysql_query($sql2);*/
		$sql3 = "SELECT * FROM media160 WHERE id= ".$_GET['id'];
		$result3 = mysql_query($sql3);
		$row3 = mysql_fetch_array($result3);
		if($row3['totalprice']!=$_POST['totalprice']){
			$data='手機簡訊媒體總金額由'.$row3['totalprice'].'改成'.$_POST['totalprice'].'<br />';
		}
		if($row3['date1']!=$date[1]){
			$data=$data.'手機簡訊媒體走期1由'.date('Ymd',$row3['date1']).'改成'.date('Ymd',$date[1]).'<br />';
		}
		if($row3['date2']!=$date[2]){
			$data=$data.'手機簡訊媒體走期1由'.date('Ymd',$row3['date2']).'改成'.date('Ymd',$date[2]).'<br />';
		}
		if($row3['date3']!=$date[3]){
			$data=$data.'手機簡訊媒體走期2由'.date('Ymd',$row3['date3']).'改成'.date('Ymd',$date[3]).'<br />';
		}
		if($row3['date4']!=$date[4]){
			$data=$data.'手機簡訊媒體走期2由'.date('Ymd',$row3['date4']).'改成'.date('Ymd',$date[4]).'<br />';
		}
		if($row3['date5']!=$date[5]){
			$data=$data.'手機簡訊媒體走期3由'.date('Ymd',$row3['date5']).'改成'.date('Ymd',$date[5]).'<br />';
		}
		if($row3['date6']!=$date[6]){
			$data=$data.'手機簡訊媒體走期3由'.date('Ymd',$row3['date6']).'改成'.date('Ymd',$date[6]).'<br />';
		}
		if($row3['date7']!=$date[7]){
			$data=$data.'手機簡訊媒體走期4由'.date('Ymd',$row3['date7']).'改成'.date('Ymd',$date[7]).'<br />';
		}
		if($row3['date8']!=$date[8]){
			$data=$data.'手機簡訊媒體走期4由'.date('Ymd',$row3['date8']).'改成'.date('Ymd',$date[8]).'<br />';
		}
		if($row3['date9']!=$date[9]){
			$data=$data.'手機簡訊媒體走期5由'.date('Ymd',$row3['date9']).'改成'.date('Ymd',$date[9]).'<br />';
		}
		if($row3['date10']!=$date[10]){
			$data=$data.'手機簡訊媒體走期5由'.date('Ymd',$row3['date10']).'改成'.date('Ymd',$date[10]).'<br />';
		}
		if($data==NULL){
			$data='手機簡訊媒體我也不知道改了什麼，有可能是版位喔';
		}
