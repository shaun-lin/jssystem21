...
$search4 = sprintf('%02d', $_GET['search4']);
...
SetExcelCellValue($objPHPExcel, [
	...
	'Y'. $number => '操作預算',
	'Z'. $number => '總收入(V-W-X)',
	'AA'. $number => $_GET['search4'].'月收入',
	'AB'. $number => $_GET['search4'].'月成本',
	'AC'. $number => $_GET['search4'].'月毛利',
	'AD'. $number => $_GET['search4'].'月毛利率',
	'AE'. $number => '總收入',
	'AF'. $number => '總成本',
	'AG'. $number => '總毛利',
	'AH'. $number => '總毛利率',
...
$specificMediaOrdinal = 0;
$extraCondition = '';
if (isset($_GET['search2']) && $_GET['search2']) {
	$specificMediaOrdinal = $_GET['search2'];
	$extraCondition = sprintf(" AND C.id IN (SELECT `campaign_id` FROM `media%d` GROUP BY `campaign_id`)", $_GET['search2']);
}
...
$sql2 = 'SELECT  CS.name as CSname ,CS.`times` as CStime ,C.* FROM campaign C 
		LEFT JOIN (SELECT * 
			FROM `campaignstatus2` 
			WHERE `data` =  "按下回簽"
			ORDER BY  `times` DESC
		) CS  ON CS.campaignid = C.id 
		WHERE C.status <> 8 AND `status` >= 2 AND `status` <= 5 
		AND (
...
if (mysql_num_rows($result2)>0){
	while($row2=mysql_fetch_array($result2)){
		...
		foreach ($rowsOrdinal as $j) {
			$sql3 = sprintf('SELECT * FROM `media%d` WHERE `campaign_id` = %d AND `cue` = 1 ORDER BY `id`;', $j, $row2['id']);
		...
		foreach ($rowsOrdinal as $j) {
			$sql3 = sprintf('SELECT * FROM `media%d` WHERE `campaign_id` = %d AND `cue` = 2 ORDER BY `id`;', $j, $row2['id']);
			$result3 = mysql_query($sql3); 
			if (mysql_num_rows($result3) > 0) {
				while ($row3 = mysql_fetch_array($result3)) {
			...
			if (mysql_num_rows($result3) > 0) {
				while ($row3 = mysql_fetch_array($result3)) {
					...
					if (isset($rowsAccounting[$j][$row3['id']]) && count($rowsAccounting[$j][$row3['id']])) {
						foreach ($rowsAccounting[$j][$row3['id']] as $accountingDetal) {
					...
					SetExcelCellValue($objPHPExcel, [
						...
						'K'. $number => $row2['CStime'] == NULL ? '':date("Y-m-d",$row2['CStime']),
						'L'. $number => $media11,
...
$sheetName = '所有媒體';
if ($specificMediaOrdinal) {
	$itemMedia = GetMedia($specificMediaOrdinal);
	$sheetName = $itemMedia['name'];
}

$excelActiveSheet->setTitle($sheetName);
$objPHPExcel->setActiveSheetIndex(0);

$xlsFilename = sprintf('%d年%d月報表-%s', $_GET['search3'], $_GET['search4'], $sheetName);
