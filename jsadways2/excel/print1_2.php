...
$objPHPExcel->getActiveSheet()->getStyle('B2:B5')->getFont()->setName('細明體');
...
//如果沒有海外媒體才執行
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 ORDER BY id';
...
if($cpc==1){
	// 標頭內容
	$number=$number+1;
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':U'.($number+1))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':U'.($number+1))->getFill()->getStartColor()->setRGB('DDDDDD'); 
	$array = array(0 => "B", 1 => "C", 2 => "D", 3 => "E", 4 => "F", 5 => "G", 6 => "H", 7 => "I", 8 => "J", 9 => "K", 10 => "L", 11 => "M", 12 => "N", 13 => "O", 14 => "P", 15 => "Q", 16 => "R", 17 => "S", 18 => "T", 19 => "U");
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('P'.$number, '佣金')
				->setCellValue('Q'.$number, '現折')
				->setCellValue('R'.$number, '利潤')
				->setCellValue('S'.$number, '媒體操作成本')
				->setCellValue('T'.$number, 'Net Cost(NTD)')
				->setCellValue('U'.$number, 'Other');
	$number=$number+1;
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('R'.$number, '利潤')
				->setCellValue('S'.$number, '媒體操作成本')
				->setCellValue('T'.$number, '售價')
				->setCellValue('U'.$number, '備註');
		...
		while($row33=mysql_fetch_array($result33)){
			...
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					...
					$price2='$'.number_format($row3['totalprice1']);
					if($row3['price2']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice2']);}
					if($row3['price3']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice3']);}
					if($row3['price4']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice4']);}
					if($row3['price5']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice5']);}
					$quantity=number_format($row3['click1']);
					if($row3['click2']!=NULL){$quantity=$quantity.'、'.number_format($row3['click2']);}
					if($row3['click3']!=NULL){$quantity=$quantity.'、'.number_format($row3['click3']);}
					if($row3['click4']!=NULL){$quantity=$quantity.'、'.number_format($row3['click4']);}
					if($row3['click5']!=NULL){$quantity=$quantity.'、'.number_format($row3['click5']);}
					...
					->setCellValue('P'.$number, '$'.number_format($row3['a1']))
					->setCellValue('Q'.$number, '$'.number_format($row3['a2']))
					->setCellValue('R'.$number, '$'.number_format($row3['a3']))
					->setCellValue('S'.$number, '$'.number_format($row3['a4']))
					->setCellValue('T'.$number, $price2)
					->setCellValue('U'.$number, $row3['others']);
					...
					$objPHPExcel->getActiveSheet()->getStyle('P'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					...
					$objPHPExcel->getActiveSheet()->getStyle('T'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('U'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
					for($j = 0; $j < count($array); $j++){
		...
		while($row33=mysql_fetch_array($result33)){
			...
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					...
					$price2='$'.number_format($row3['totalprice1']);
					if($row3['totalprice2']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice2']);}
					if($row3['totalprice3']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice3']);}
					if($row3['totalprice4']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice4']);}
					if($row3['totalprice5']!=NULL){$price2=$price2.'、$'.number_format($row3['totalprice5']);}
					if(isset($row3['click1'])){
						$quantity=number_format($row3['click1']);
						if($row3['click2']!=NULL){$quantity=$quantity.'、'.number_format($row3['click2']);}
						if($row3['click3']!=NULL){$quantity=$quantity.'、'.number_format($row3['click3']);}
						if($row3['click4']!=NULL){$quantity=$quantity.'、'.number_format($row3['click4']);}
					...
					if($price2 == 0 && $row3['totalprice'] != 0){
						$price2 = $row3['totalprice'];
					}
...
if($webad==1){
	// 標頭內容
	$number=$number+2;
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':S'.($number+1))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':S'.($number+1))->getFill()->getStartColor()->setRGB('DDDDDD');
	$array = array(0 => "B", 1 => "C", 2 => "D", 3 => "E", 4 => "F", 5 => "G", 6 => "H", 7 => "I", 8 => "J", 9 => "K", 10 => "L", 11 => "M", 12 => "N", 13 => "O", 14 => "P", 15 => "Q", 16 => "R", 17 => "S");
	for($j = 0; $j < count($array); $j++){
		for($i=$number;$i<=($number+1);$i++){
			$objPHPExcel->getActiveSheet()->getStyle($array[$j].$i)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
			$objPHPExcel->getActiveSheet()->getStyle($array[$j].$i)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('N'.$number, '佣金')
				->setCellValue('O'.$number, '現折')
				->setCellValue('P'.$number, '利潤')
				->setCellValue('Q'.$number, '媒體操作成本')
				->setCellValue('R'.$number, 'Net Cost(NTD)');
	$number=$number+1;
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('P'.$number, '利潤')
				->setCellValue('Q'.$number, '媒體操作成本')
				->setCellValue('R'.$number, '售價')
				->setCellValue('S'.$number, '備註');
		...
		while($row33=mysql_fetch_array($result33)){
			...
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					...
					->setCellValue('N'.$number, '$'.number_format($row3['a1']))
					->setCellValue('O'.$number, '$'.number_format($row3['a2']))
					->setCellValue('P'.$number, '$'.number_format($row3['a3']))
					->setCellValue('Q'.$number, '$'.number_format($row3['a4']))
					->setCellValue('R'.$number, '$'.number_format($row3['totalprice']))
					->setCellValue('S'.$number, $row3['others']);
					$totalprice=$totalprice+$row3['totalprice'];
					$objPHPExcel->getActiveSheet()->getStyle('B'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('H'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('J'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
if($cpa==1){
	...
	if ((!in_array($chatBotMediaId, $rowsOrdinal) && count($rowsOrdinal)) || (in_array($chatBotMediaId, $rowsOrdinal) && count($rowsOrdinal) > 1)) {
		// 標頭內容
		$number=$number+2;
		$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':R'.($number+1))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':R'.($number+1))->getFill()->getStartColor()->setRGB('DDDDDD');
		$array = array(0 => "B", 1 => "C", 2 => "D", 3 => "E", 4 => "F", 5 => "G", 6 => "H", 7 => "I", 8 => "J", 9 => "K", 10 => "L", 11 => "M", 12 => "N", 13 => "O", 14 => "P", 15 => "Q", 16 => "R");
		for($j = 0; $j < count($array); $j++){
			for($i=$number;$i<=($number+1);$i++){
				$objPHPExcel->getActiveSheet()->getStyle($array[$j].$i)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
				$objPHPExcel->getActiveSheet()->getStyle($array[$j].$i)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('M'.$number, '佣金')
					->setCellValue('N'.$number, '現折')
					->setCellValue('O'.$number, '利潤')
					->setCellValue('P'.$number, '媒體操作成本')
					->setCellValue('Q'.$number, 'Net Cost(NTD)');
		$number=$number+1;

		$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue('B'.$number, '網站')
					->setCellValue('C'.$number, 'CPA內容1')
					->setCellValue('D'.$number, '版位')
					->setCellValue('E'.$number, '規格')
					->setCellValue('F'.$number, '格式')
					->setCellValue('G'.$number, '輪替/固定')
					...
					->setCellValue('O'.$number, '利潤')
					->setCellValue('P'.$number, '媒體操作成本')
					->setCellValue('Q'.$number, '售價')
					->setCellValue('R'.$number, '備註');
		...
		while($row33=mysql_fetch_array($result33)){
			...
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					...
					->setCellValue('M'.$number, '$'.number_format($row3['a1']))
					->setCellValue('N'.$number, '$'.number_format($row3['a2']))
					->setCellValue('O'.$number, '$'.number_format($row3['a3']))
					->setCellValue('P'.$number, '$'.number_format($row3['a4']))
					->setCellValue('Q'.$number, '$'.number_format($row3['a1']+$row3['a2']+$row3['a3']+$row3['a4']))
					->setCellValue('R'.$number, $row3['others']);
					$totalprice=$totalprice+$row3['a1']+$row3['a2']+$row3['a3']+$row3['a4'];
					$objPHPExcel->getActiveSheet()->getStyle('B'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('H'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('J'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('K'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
if($monipla==1){
	// 標頭內容
	$number=$number+2;
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':Q'.($number))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':Q'.($number))->getFill()->getStartColor()->setRGB('DDDDDD');
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('L'.$number, '佣金')
				->setCellValue('M'.$number, '現折')
				->setCellValue('N'.$number, '利潤')
				->setCellValue('O'.$number, '媒體操作成本')
				->setCellValue('P'.$number, '售價')
				->setCellValue('Q'.$number, '日期');
		...
		while($row33=mysql_fetch_array($result33)){
			...
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					...
					->setCellValue('L'.$number, '$'.number_format($row3['a1']))
					->setCellValue('M'.$number, '$'.number_format($row3['a2']))
					->setCellValue('N'.$number, '$'.number_format($row3['a3']))
					->setCellValue('O'.$number, '$'.number_format($row3['a4']))
					->setCellValue('P'.$number, '$'.number_format($row3['totalprice']))
					->setCellValue('Q'.$number, $datedate);
					$totalprice=$totalprice+$row3['totalprice'];
					$objPHPExcel->getActiveSheet()->getStyle('B'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('H'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('J'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$ad=0;
//網站廣告媒體
$sql3='SELECT * FROM media18 WHERE campaign_id = '.$_GET['id'].' AND cue=2 ORDER BY id';
$result3=mysql_query($sql3); 
if (mysql_num_rows($result3)>0){
	while($row3=mysql_fetch_array($result3)){
		$ad=1;
...
if($ad==1){
	// 標頭內容
	$number=$number+2;
	$objPHPExcel->getActiveSheet()->mergeCells('B'.$number.':G'.$number);
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':G'.($number+1))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':G'.($number+1))->getFill()->getStartColor()->setRGB('DDDDDD');
	...
	if (mysql_num_rows($result3)>0){
		while($row3=mysql_fetch_array($result3)){
			...
			->setCellValue('C'.$number, number_format($row3['price']))
			->setCellValue('D'.$number, $row3['quantity'])
			->setCellValue('E'.$number, number_format($row3['totalprice']))
			->setCellValue('G'.$number, $row3['other']);
...
if($blog==1){
	...
	if (mysql_num_rows($result3)>0){
		while($row3=mysql_fetch_array($result3)){
			...
			->setCellValue('D'.$number, '$'.number_format($row3['price']));
			$totalprice=$totalprice+$row3['price'];
			$objPHPExcel->getActiveSheet()->getStyle('B'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			for($j = 0; $j < count($array); $j++){
				$objPHPExcel->getActiveSheet()->getStyle($array[$j].$number)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
if (mysql_num_rows($result33)>0){
	while($row33=mysql_fetch_array($result33)){
		...
		$result3=mysql_query($sql3); 
		if (mysql_num_rows($result3)>0){
			$sql_sp = 'SELECT * FROM media94 where campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPM" ORDER BY id';
			$result_sp = mysql_query($sql_sp); 
			if (mysql_num_rows($result_sp)>0){
				$cpm=1;
			}

			$sql_sp = 'SELECT * FROM media94 where campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPC" ORDER BY id';
			$result_sp = mysql_query($sql_sp); 
			if (mysql_num_rows($result_sp)>0){
				$cpc=1;
			}

			$sql_sp = 'SELECT * FROM media94 where campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPV" ORDER BY id';
			$result_sp = mysql_query($sql_sp); 
			if (mysql_num_rows($result_sp)>0){
				$cpv=1;
			}

			$sql_sp = 'SELECT * FROM media94 where campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPI" ORDER BY id';
			$result_sp = mysql_query($sql_sp); 
			if (mysql_num_rows($result_sp)>0){
				$cpi=1;
			}

			$sql_sp = 'SELECT * FROM media94 where campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPT" ORDER BY id';
			$result_sp = mysql_query($sql_sp); 
			if (mysql_num_rows($result_sp)>0){
				$cpt=1;
			}

			$sql_sp = 'SELECT * FROM media94 where campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "網站廣告" ORDER BY id';
			$result_sp = mysql_query($sql_sp); 
			if (mysql_num_rows($result_sp)>0){
				$cpa=1;
			}
...
if($cpm==1){
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPM" ORDER BY id';
			$result3=mysql_query($sql3); 
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					$number=$number+1;
...
if($cpc==1){
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPC" ORDER BY id';
			$result3=mysql_query($sql3); 
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					$number=$number+1;
...
if($cpv==1){
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPV" ORDER BY id';
			$result3=mysql_query($sql3); 
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					$number=$number+1;
...
if($cpi==1){
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPI" ORDER BY id';
			$result3=mysql_query($sql3); 
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					$number=$number+1;
...
if($cpt==1){
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "CPT" ORDER BY id';
			$result3=mysql_query($sql3); 
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					$number=$number+1;
...
if($cpa==1){
	...
	if (mysql_num_rows($result33)>0){
		while($row33=mysql_fetch_array($result33)){
			$sql3='SELECT * FROM media'.$row33['id'].' WHERE campaign_id = '.$_GET['id'].' AND cue=2 AND media_type = "網站廣告" ORDER BY id';
			$result3=mysql_query($sql3); 
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					$number=$number+1;
		...
		while($row33=mysql_fetch_array($result33)){
			...
			if (mysql_num_rows($result3)>0){
				while($row3=mysql_fetch_array($result3)){
					...
					->setCellValue('S'.$number, '$'.number_format($row3['a4']))
					->setCellValue('T'.$number, $price2)
					->setCellValue('U'.$number, '「海外媒體」'.$row3['others']);
					$totalprice=$totalprice+$row3['totalprice'];
					$objPHPExcel->getActiveSheet()->getStyle('B'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('D'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('H'.$number)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
$number=$number+2;
/*$objPHPExcel->getActiveSheet(0)->getRowDimension($number)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+1)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+2)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+3)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+4)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+5)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+7)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+8)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+9)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+10)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+11)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+12)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+13)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+14)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+15)->setRowHeight(50);
$objPHPExcel->getActiveSheet(0)->getRowDimension($number+16)->setRowHeight(50);*/
$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':B'.($number+12))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('B'.$number.':B'.($number+12))->getAlignment()->setvertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
...
$xlsFilename = 'JS_'.$filename1.'_'.$row2['name'].'_'. date('md', $row2['date11']) .'-'. date('md', $row2['date22'])  .'_'.$timetimes.'.xls';
