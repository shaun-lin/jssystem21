<?php
	...
	$objMedia = CreateObject('Mtype');


	$mtypeodinal = GetVar('Mtype');
    $sizeformatConditions =IsId($meidaOrdinal) ? sprintf('`id` = %d', $meidaOrdinal) : 'id IN (SELECT `id` FROM `mtype`) order by name' ;
                    ...
                    <div class="row-fluid">
                        <div class="box span12">
                            <div class="box-header well" data-original-title>
                                <h2><i class="fa fa-cubes"></i> 模板維護作業</h2>
                            </div>

                            <div class="box-content">
                                <div class="dropdown">
                                    <button class="dropbtn" style="border-radius: 4px; margin-right: 12px; margin-bottom: 12px;" onclick="window.location.href='mtype_list.php';">
                                ...
                                <? endforeach; ?>
                                <br/><br/>
                                <a class="btn btn-primary" href="mtype_edit.php">
                                    ...
                                    <tbody>
                                        <? foreach ($objSizeformat->searchAll($sizeformatConditions) as $itemFormat) : ?>
                                            <tr>
                                                ...
                                                <td><?= $itemFormat['dashboard']; ?></td>
                                                <td class="center"><?= str_replace('\\n', "<br/>", ($itemFormat['display']))=="1"?"顯示":"不顯示"; ?></td>
                                                <? if (IsPermitted('superuser') || in_array($_SESSION['departmentid'], [21, 22])) : ?>
                                                    <td class="center">
                                                        <a class="btn btn-info" href="mtype_edit.php?id=<?= $itemFormat['id']; ?>">
                                                            <i class="icon-edit icon-white"></i>  
                                                            Edit                                            
                                                        </a>
                                                        <a class="btn btn-danger" onclick='return confirm("點擊「確定」即刪除所選擇的資料。")'  href="mtype_del.php?id=<?= $itemFormat['id']; ?>">
