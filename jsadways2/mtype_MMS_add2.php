...
if ($_POST['position'] == 1) {
    ...
    $_POST['position'] = '簡訊廣告-基本篩選';
} elseif ($_POST['position'] == 3) {
    $_POST['position'] = '簡訊廣告-特殊篩選';
} elseif ($_POST['position'] == 4) {
    $_POST['position'] = 'Happygo';
} elseif ($_POST['position'] == 5) {
    $_POST['position'] = '台哥大LBS';
} elseif ($_POST['position'] == 6) {
    $_POST['position'] = '中華、遠傳LBS';
}
for ($i = 1; $i <= 10; $i++) {
...
$sqlmedia = "SELECT * FROM media WHERE id=160";
...
$sql2 = 'INSERT INTO media160(item_seq,campaign_id,cue,website,channel,position,format1,format2,wheel,date1,date2,date3,date4,date5,date6,date7,date8,date9,date10,days,due,quantity,price,totalprice,times,others,a,a0,a1,a2,a3,a4,a5,days1,days2,days3,days4,days5,items2,items3) VALUES(' . $autoSerialNumberA . ',' . $_GET['id'] . ',' . $_GET['cue'] . ',"' . $_GET['media_name'] . '","' . $_POST['channel'] . '","' . $_POST['position'] . '","' . $_POST['format1'] . '","' . $_POST['format2'] . '","' . $_POST['wheel'] . '",' . $date[1] . ',' . $date[2] . ',' . $date[3] . ',' . $date[4] . ',' . $date[5] . ',' . $date[6] . ',' . $date[7] . ',' . $date[8] . ',' . $date[9] . ',' . $date[10] . ',"' . $_POST['days'] . '","' . $_POST['due'] . '","' . $_POST['quantity'] . '","' . $_POST['price'] . '","' . $_POST['totalprice'] . '",' . time() . ',"' . $_POST['others'] . '","' . $a . '","' . $a0 . '","' . $a1 . '","' . $a2 . '","' . $a3 . '","' . $a4 . '","' . $a5 . '","' . $_POST['days1'] . '","' . $_POST['days2'] . '","' . $_POST['days3'] . '","' . $_POST['days4'] . '","' . $_POST['days5'] . '","' . $TypeItem . '","' . $_POST['SelectSystem'] . '")';
mysql_query($sql2);
AddMediaMapping("media160", $_GET['id'], mysql_insert_id());
...
if ($_POST['samecue'] == 1) {
    ...
    $sqlnew = "SELECT * FROM media160 ORDER BY id DESC LIMIT 1;";
    ...
    $sql2 = 'INSERT INTO media160(item_seq,campaign_id,cue,website,channel,position,format1,format2,wheel,ctr,date1,date2,date3,date4,date5,date6,date7,date8,date9,date10,days,due,quantity,price,totalprice,times,others,a,a0,a1,a2,a3,a4,days1,days2,days3,days4,days5,items2,items3) VALUES(' . $autoSerialNumberB . ',' . $_GET['id'] . ',2,"' . $_GET['media_name'] . '","' . $_POST['channel'] . '","' . $position . '","' . $_POST['format1'] . '","' . $_POST['format2'] . '","' . $_POST['wheel'] . '","' . $_POST['ctr'] . '",' . $date[1] . ',' . $date[2] . ',' . $date[3] . ',' . $date[4] . ',' . $date[5] . ',' . $date[6] . ',' . $date[7] . ',' . $date[8] . ',' . $date[9] . ',' . $date[10] . ',"' . $_POST['days'] . '","' . $_POST['due'] . '","' . $_POST['quantity'] . '","' . $_POST['price'] . '","' . $_POST['totalprice'] . '",' . time() . ',"' . $_POST['others'] . '","' . $a . '","' . $a0 . '","' . $a1 . '","' . $a2 . '","' . $a3 . '","' . $a4 . '","' . $_POST['days1'] . '","' . $_POST['days2'] . '","' . $_POST['days3'] . '","' . $_POST['days4'] . '","' . $_POST['days5'] . '","' . $TypeItem . '","' . $_POST['SelectSystem'] . '")';
    mysql_query($sql2);
    AddMediaMapping("media160", $_GET['id'], mysql_insert_id());
...
$row1 = mysql_fetch_array($result1);
if ($row1['status'] == 5) {
    $sql2 = 'INSERT INTO campaignstatus2(name,data,times,campaignid) VALUES("' . $_SESSION['username'] . '","新增手機簡訊媒體",' . time() . ',' . $_GET['id'] . ')';
    mysql_query($sql2);
}
