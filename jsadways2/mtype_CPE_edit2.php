<?php
	...
	if ($row1['status'] == 5) {
		$sql3 = sprintf("SELECT * FROM `media171` WHERE `id` = %d;",  $_GET['id']);
		$db->query($sql3);
		$row3 = $db->next_record();
	...
	if ($_POST['gearing'] == 1) {
		...
		if ($campaign_row == 2) {
			...
			$sql2 = "UPDATE `media171` SET ". implode(', ', $fieldsStatement) ." WHERE `campaign_id`	= ". ((int)$_GET['campaign']) .";";
			$db->query($sql2);
		}
	}
	...
	$sql2 = "UPDATE `media171` SET ". implode(', ', $fieldsStatement) ." WHERE `id`	= ". ((int)$_GET['id']) .";";
	$db->query($sql2);
