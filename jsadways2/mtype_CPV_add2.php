<?php
	...
	require_once 'mtype_CPV_definition.php';
	...
	$sql2 = GenSqlFromArray($addDataForCue1, "media154", 'insert');
	$db->query($sql2);
	...
	if ($_POST['samecue'] == 1) {
		...
		$sqlnew = sprintf("SELECT * FROM `media154` ORDER BY `id` DESC LIMIT 1;", $mediaOrdinal);
		$db->query($sqlnew);
		...
		$sql2 = GenSqlFromArray($addDataForCue2, "media154", 'insert');
		$db->query($sql2);
		AddMediaMapping("media154", $_GET['id'], $db->get_last_insert_id());
